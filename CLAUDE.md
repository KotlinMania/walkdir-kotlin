# Claude Code Project Instructions — walkdir-kotlin

## Project Overview

This is **walkdir-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`walkdir`](https://crates.io/crates/walkdir).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/walkdir/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/walkdir>

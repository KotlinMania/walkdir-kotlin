...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Walkdir.swiftmodule/`
...
//      `import Walkdir` above would fail at compile time.
//
//   2. The static archive `libWalkdir.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Walkdir" }` and
//      `flattenPackage = "io.github.kotlinmania.walkdir"` configuration in
...
//      via the `WalkdirLibrary` product.
//
...
final class WalkdirExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Walkdir swift module imported cleanly")
    }
}
